import type { ChatParticipant, Command, CustomEditor, GenerateOptions, Language, Manifest, Property, TaskDefinition } from './types'
...
export function generateDTS(packageJson: Manifest, options: GenerateOptions = {}) {
  ...
  if (packageJson.contributes?.taskDefinitions?.length) {
    const taskDefs = (packageJson.contributes.taskDefinitions || []).filter(
      (t: TaskDefinition) => t.type,
    ) as TaskDefinition[]
    if (taskDefs.length) {
      lines.push(
        '',
        ...commentBlock('Type union of all task definitions'),
        'export type TaskType =',
        ...taskDefs.map((t: TaskDefinition) => `  | ${JSON.stringify(t.type)}`),
      )
      lines.push(
        '',
        'export interface TaskPropertiesMap {',
        ...taskDefs.flatMap((t: TaskDefinition) => {
          const props = t.properties || {}
          const entries = Object.entries(props)
          if (entries.length === 0) {
            return [`  ${JSON.stringify(t.type)}: {},`]
          }
          return [
            `  ${JSON.stringify(t.type)}: {`,
            ...entries.map(([key, value]: [string, Property]) =>
              `    ${JSON.stringify(key)}: ${typeFromSchema(value)},`,
            ),
            '  },',
          ]
